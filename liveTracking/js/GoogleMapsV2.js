...
$(document).ready(function () {
    ...
    $(document).on('click', '.route-box', function (event) {
        ...
        var id = route_name.replace("Shuttle ", "");
...
function showETA(EtaMessage) {
    $(".eta-details").show();
    $(".eta-details").html("");
    htmlStr = '<table><caption>Estimated Time Of Arrival</caption' +
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (i = 0; i < response.length; i++) {
                route = response[i];
                var htmlStr = '<div class="route-box" id="' + route.id + '"><a href="#"><img src="img/route.png" alt="Routes"><div class="desc">' + route.shuttleName + '</div></a></div>';
...
function showOnMap(route) {
    ...
    displayRoute(startPoint, endPoint, directionsService,
        directionsDisplay, markers, route.shuttleName, route);
...
function displayRoute(origin, destination, service, display, waypoints, markerTitle, shuttleData) {
    var id = markerTitle.replace("Shuttle ", "");
    debugger;
    randomid = id;
    service.route({
        origin: origin,
        ...
        if (status === 'OK') {
                ...
                for (k = 0; k < shuttleData.wayPoints.length; k++) {
                    ...
                    if (wayPoint.type == 'Stop' || wayPoint.type == 'End') {
                        ...
                        localInfoWindows[indexK] = new google.maps.InfoWindow({
                            content: "<div id='shuttle" + id + "" + indexK + "'>Loading ETA..!</div>"
                        });
            ...
            animateMarker(marker[id], arrayOfLatLng, speed, Markers[id], markerTitle, id);
        } else {
...
function RegisterClickEvent(stopMarker2, show, id) {
    ...
    google.maps.event.addListener(stopMarker2, 'click', (function (stopMarker2, opened, id) {
        return function () {
            ...
            else if (EtaMarker.customInfo.name == thisMarker.customInfo.name && opened) {
                opened = !opened;
            }
            if (opened) {
...
function animateMarker(marker, coords1d, km_h, Markers, markerTitle, id) {
    ...
    marker.addListener('position_changed', function () {
        ...
        try {
            // var stop_markers = Markers.filter(function (obj) {
            //     return (obj.customInfo.type == 'Stop' || obj.customInfo.type == 'end') && obj.customInfo.routeId == id;
            // });
            for (indexIJ = 0; indexIJ < Markers.length; indexIJ++) {
                if (Markers[indexIJ].customInfo.name == EtaMarker.customInfo.name) {
                    var distance = CalculateDistance(coords1d, marker.position, Markers[indexIJ].position);
                    if (distance != 0) {
                        if (distance == -1) {
                            ...
                            if (time == '0 sec' || time == '1 sec' || time == '2 sec' || time == '3 sec') {
                                ...
                                document.getElementById("etaMessage").innerHTML = 'Shuttle  ' + id + ' will arrive in ' + time;
